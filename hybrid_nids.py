    ...
    def monitor_suricata_file(self, file_path, output_file=None):
        ...
        try:
            while True:
                ...
                # Force incremental analysis of active sessions periodically
                if self.incremental_analysis and current_time - last_incremental_check > 5.0:  # Every 5 seconds
                    # Run incremental analysis directly on active sessions
                    if hasattr(self, 'incremental_analyzer') and self.incremental_analyzer:
                        results = self.incremental_analyzer.analyze_active_sessions(self.session_manager.sessions)
                        incremental_analyses += len(self.session_manager.sessions)
                        incremental_alerts += len(results)
                ...
                if current_time - last_stats_update > 60:
                    ...
                    # Add incremental analysis stats if enabled
                    if self.incremental_analysis:
                        logger.info(f"Incremental analyses: {incremental_analyses} | Incremental alerts: {incremental_alerts}")
                        
                        # Get current stats from incremental analyzer
                        if hasattr(self, 'incremental_analyzer') and self.incremental_analyzer:
                            inc_stats = self.incremental_analyzer.get_stats()
                            logger.info(f"Active tracked flows: {inc_stats.get('active_tracked_flows', 0)} | "

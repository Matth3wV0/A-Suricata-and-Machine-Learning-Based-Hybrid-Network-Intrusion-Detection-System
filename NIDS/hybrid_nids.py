...
class HybridNIDS:
    ...
    def __init__(self, model_dir='./model', telegram_enabled=False):
        ...
        self.alerter = TelegramAlerter() if telegram_enabled else None
        ...
        if telegram_enabled:
            try:
                # Initialize Telegram client directly
    ...
    def handle_alert(self, alert_data):
        ...
        """
        try:
            # Format alert message with ASCII characters only
            ...
            if self.telegram_enabled and self.telegram_client:
                try:
                    # Send the message directly with the client
                    import asyncio
...
def main():
    ...
    if args.telegram and nids.alerter and hasattr(nids.alerter, 'loop') and nids.alerter.loop and nids.alerter.loop.is_running():
        logger.info("Waiting for pending Telegram operations to complete...")
